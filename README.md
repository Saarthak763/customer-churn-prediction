...
_(Reproduce with `python src/data_prep.py && python src/train.py`)_
...
5. **Explainability** — SHAP values to show _why_ each customer is flagged as high-risk
...
An interactive dashboard built on top of the model's predictions,
allowing business users to explore churn risk without running any code.

**Default view — all customers:**
![Dashboard](reports/figures/powerbi_dashboard.png)

**Filtered to month-to-month contracts only:**
![Dashboard Filtered](reports/figures/powerbi_dashboard_filtered.png)

Built using `churn_powerbi.csv` — generated by `src/export_for_powerbi.py`.
